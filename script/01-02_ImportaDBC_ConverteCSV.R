...
lapply(data.raw.lista,
                   function(x){
                     aaaa = str_sub(x,start = -12, end = -5)
                     
                     b = read.dbc(x) %>% setDT()
                     
                     fwrite(b, file = paste0("./data/raw/",aaaa,".csv")
                     )
                   }
)
...
# apaga os arquivos .dbc, mantendo apeanas os correspondentes .csv
lapply(data.raw.lista, function(x){
  unlink(x)
})
}
